...
static int wait_for_cmd_done (void)
{
	...
	udelay(5000);//czm add by 2012-07-07
...
static void ClearCommandCompleteStatus(void)
{
	...
	s3c_hsmmc_writew((1 << 0), HM_NORINTSTS);//czm add by 2012-07-07
...
static void set_clock (uint clksrc, uint div)
{
	...
	//s3c_hsmmc_writel(0xC0000100 | (clksrc << 4)|(1<<14) |(1<<15), HM_CONTROL2);	// feedback control off
	s3c_hsmmc_writel(0xC0000100 | (clksrc << 4), HM_CONTROL2);	// feedback control off
	s3c_hsmmc_writel((0<<31) | (0<<23) | (0<<15) | (0<<7) , HM_CONTROL3);
	//s3c_hsmmc_writel((1<<31) | (1<<23) | (1<<15) | (1<<7) , HM_CONTROL3);//czm add by 2012-07-07
	...
	dbg("HM_CLKCON  (%04x) = 0x%04x\n", HM_CLKCON,s3c_hsmmc_readw(HM_CLKCON));
...
static void set_mmc_speed (uint eSDSpeedMode)
{
	...
	dbg("change to high speed mode: %d",ucSpeedMode);
...
static int set_sd_ocr (void)
{
	...
	issue_command(SD_APP_OP_COND, 0, 0, MMC_RSP_R3);
...
//	writel(reg | (1<<27), SCLK_GATE);
	...
	/* add logic to detect card type ie: sdhc vs. normal sd cards 1.x */
	...
	mmc_dev.if_type = IF_TYPE_UNKNOWN;
        ...
        issue_command(MMC_CMD_IF_COND, ((1 << 8) | 0xAA),0, 0x01);
	uint resp = s3c_hsmmc_readl(HM_RSPREG0);
	dbg("cmd 8 resp: %08x\n",resp);
        if (resp == 0) {
             /*
              ...
              */
             mmc_dev.if_type = IF_TYPE_SDHC;
	     movi_hc = 1;//added by Chao Ling 2014/2/28 to recognize the correct sd capacity	
             puts("mmc: SD 2.0 or later card found\n");

             /* Check if the card supports this voltage */
             if (resp != ((1 << 8) | 0xAA)) {
                     dbg("mmc: Invalid voltage range\n");
                     return -1;
             }
        }
